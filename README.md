...
在 **设置 → 软件更新** 检查版本。首次配置自己的 GitHub Fork 和限定仓库的更新令牌后，可点击“一键更新”，同步兼容分支并等待已连接的部署平台发布，完成后自动载入。已有自定义代码时会停止自动更新，不强制覆盖。旧部署需先手动更新一次才能获得此入口。详见 [更新教程](docs/software-update.md)。
